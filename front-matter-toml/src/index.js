...
/**
 * Parses TOML front matter and returns the extracted data and content.
 ...
 * @throws {Error} - If the front matter format is invalid or TOML parsing fails.
 */
export function parse(text) {
  ...
  if (!match) {
    throw new Error(
      `Invalid front matter format. Ensure the document starts with "+++" and follows TOML syntax.`,
    )
  }
  ...
  try {
    ...
    throw new Error(`TOML Parsing Error: ${error.message}`, { cause: error })
  }
}

/**
 * Serializes data and content into a formatted TOML front matter string.
...
export function serialize(data, content) {
  ...
  try {
    const frontMatter = toml.stringify(data).trim()

    return `+++\n${frontMatter}\n+++\n${content}`
  } catch (error) {
    throw new Error(`TOML Serialization Error: ${error.message}`, {
      cause: error,
    })
  }
}
...
/**
 * Validates whether a given text has properly formatted front matter and parsable TOML.
 * @param {string} text - The full document to validate.
 * @returns {boolean} - `true` if the format is valid and TOML is parsable, otherwise `false`.
 */
export function validate(text) {
  ...
  try {
    toml.parse(match[1]) // Validate TOML
